...
const SkeletonLoader = () => (
  <div className="animate-pulse flex flex-col gap-4">
    ...
    <div className="h-10 w-1/2 bg-gray-200 dark:bg-gray-700 rounded"></div>
  </div>
);
...
function Productos() {
  ...
  // Simulate loading time (remove after actual data fetching)
  useEffect(() => {
    setTimeout(() => {
    ...
    <main className="flex justify-center flex-col items-center mb-10">
      ...
      <article className="grid grid-cols-9 lg:gap-10 xs:gap-28 w-full container mt-10">
        ...
        <section className="col-span-9 lg:col-span-6 p-3 min-h-[35vh]">
          {loading ? (
            <SkeletonLoader /> // Show Skeleton loader while loading is true
          ) : !ida || (!vuelta && hasVueltas) ? (
            !ida ? (
              <div>
                <Trenes setTren={setIda} trenes={trenes.idas} tipo={"idas"} />
              </div>
            ) : hasVueltas ? (
              <div>
                <Trenes
