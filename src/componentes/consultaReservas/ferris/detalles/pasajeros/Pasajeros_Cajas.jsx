    ...
    <div>
      <div className="tw-grid sm:tw-grid-cols-2 lg:tw-grid-cols-2 xl:tw-grid-cols-4 tw-gap-6">
        {pasajeros.map((pasajero, index) => (
          <div
            key={index}
            className="tw-flex tw-flex-col tw-h-fit tw-gap-2 tw-p-4 tw-rounded-2xl tw-border tw-border-slate-200 dark:tw-border-slate-700 dark:tw-bg-slate-900 tw-shadow-sm hover:tw-shadow-lg tw-transition-shadow"
          >
            <div className="tw-flex tw-items-center tw-gap-3 tw-border-b tw-border-slate-100 dark:tw-border-slate-700">
              {pasajero.sexo === "Hombre" ? (
                <FaMale className="tw-text-3xl tw-text-secondary dark:tw-text-indigo-400" />
              ) : (
                <FaFemale className="tw-text-3xl tw-text-secondary dark:tw-text-indigo-400" />
              )}
            ...
            <ul className="tw-text-sm tw-space-y-1 tw-text-slate-500 dark:tw-text-slate-300 tw-ml-2">
              ...
              <li className="tw-flex tw-items-center tw-font-normal tw-text-xs tw-gap-2">
                <span className="tw-font-semibold">Caducidad:</span>
                {pasajero.caducidad}
              </li>
              {pasajero.familiaNum === 1 && (
                <li className="tw-flex tw-items-center tw-font-normal tw-text-xs tw-gap-2">
                  Familia Númerosa
                </li>
              )}
              {pasajero.residente === 1 && (
                <li className="tw-flex tw-items-center tw-font-normal tw-text-xs tw-gap-2">
                  Residente
                </li>
              )}
